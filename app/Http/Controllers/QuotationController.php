...
class QuotationController extends Controller
{
    ...
    public function create(Request $request)
    {
		...
		$disc_res = DB::table('config_discount')->where('is_delete',0)->orderby('dealerid','asc')->get();
		$cfg_res = DB::table('config_fields')->where('is_delete',0)->where('dealerid',$dealerid)->get();
		...
		$createpagehidden = isset($request->createpagehidden) ? $request->createpagehidden : '';
		$quote_type = isset($request->quote_type) ? $request->quote_type : 'Customer';
		...
		$cons_act_weight = isset($request->cons_act_weight) ? $request->cons_act_weight : 0;
		...
		$inr_usd = $aed_usd = $cst_fwd_distr_buyer = 0;
		if($cfg_res->count() > 0)
		{
			foreach($cfg_res as $res)
			{
				...
				if($res->para_name == 'cst_fwd_distr_buyer')
				{
					$cst_fwd_distr_buyer = $res->para_value;
					$cst_fwd_distr_buyer = number_format($cst_fwd_distr_buyer, 2, '.', '');
				}
			}
		}
		...
		$exres = DB::table('config_exchange_update')->whereRaw("updated_at >= '".$today_date."'")->where('isupdate',1)->first();
		$exrate_updated = FALSE;
		if($exres)
		{
			$exrate_updated = TRUE;
		}
		...
		//CUST LOCATION
		$cust_locs = DB::table('config_customer_location')->select('*')->where('is_delete',0)->get();
		//CUST LOCATION
        ...
        return view('quotations.create', compact('dtobj','op_name','disc_res','cfg_res','request','maxshipvol','quote_type','sheet_type','shipment_type','cargo_type','currency_type','cons_act_weight','showtable','inr_usd','aed_usd','sheet_type_arr','exrate_updated','createpagehidden','cst_fwd_distr_buyer','cust_locs','dealers'));
    }
    ...
    public function store(Request $request)
    {
		...
		$cons_act_weight = isset($request->cons_act_weight2) ? $request->cons_act_weight2 : 0;
		...
		$total_ship_vol_qty_h = isset($request->total_ship_vol_qty_h) ? $request->total_ship_vol_qty_h : 0;
		$total_ship_vol_qty_h = ($total_ship_vol_qty_h == '') ? 0 : floatval($total_ship_vol_qty_h);
		...
		$quotettlcost = isset($request->quotettlcost) ? $request->quotettlcost : 0;
		$quotettlcost = ($quotettlcost == '') ? 0 : floatval($quotettlcost);
		
		$quotettlcost2 = isset($request->quotettlcost2) ? $request->quotettlcost2 : 0;
		$quotettlcost2 = ($quotettlcost2 == '') ? 0 : floatval($quotettlcost2);
		
		$gwiltotal_inr = isset($request->gwiltotal_inr) ? $request->gwiltotal_inr : 0;
		$gwiltotal_inr = ($gwiltotal_inr == '') ? 0 : floatval($gwiltotal_inr);
		
		$colgtotalprice = isset($request->colgtotalprice) ? $request->colgtotalprice : 0;
		$colgtotalprice = ($colgtotalprice == '') ? 0 : floatval($colgtotalprice);
		
		$cst_fwd_distr_buyer = isset($request->cst_fwd_distr_buyer) ? $request->cst_fwd_distr_buyer : 0;
		$cst_fwd_distr_buyer = ($cst_fwd_distr_buyer == '') ? 0 : floatval($cst_fwd_distr_buyer);
		$cst_fwd_distr_buyer = round($cst_fwd_distr_buyer,4);
		
		$cust_loc = isset($request->cust_loc2) ? $request->cust_loc2 : 0;
		$cust_loc = ($cust_loc == '') ? 0 : floatval($cust_loc);
		$cust_loc_text = isset($request->cust_loc_text2) ? $request->cust_loc_text2 : '';

		$dealer_name2 = isset($request->dealer_name2) ? $request->dealer_name2 : 0;
		$dealer_name = ($dealer_name2 == '') ? 0 : intval($dealer_name2);
		$dealer_name_text = isset($request->dealer_name_text2) ? $request->dealer_name_text2 : '';
		...
		if($total_ship_vol_qty > 0)
		{
			...
			$prod_data_main = array(
				...
				'total_ship_vol_qty_h' => $total_ship_vol_qty_h,
				'exp_cred_cost_thr_months' => $exp_cred_cost_thr_months,
				'estd_miniman_cst' => $estd_miniman_cst,
				'quotettlcost' => $quotettlcost,
				'quotettlcost2' => $quotettlcost2,
				'gwiltotal_inr' => $gwiltotal_inr,
				'colgtotalprice' => $colgtotalprice,
				'inr_usd_ex' => $inr_usd,
				'aed_usd_ex' => $aed_usd,
				'cst_fwd_distr_buyer' => $cst_fwd_distr_buyer,
				'cust_loc' => $cust_loc_text,
				'cust_loc_fwd_val' => $cust_loc,
				'dealer_name' => $dealer_name,
				'dealer_name_text' => $dealer_name_text,
			...
			{				
				...
				foreach($request->prodtag as $k1=>$prodtag)
				{
					if($prodtag != '')
					{
						...
						$subttlcost = isset($request->subttlcost[$k1]) ? $request->subttlcost[$k1] : 0;
						$subttlcost2 = isset($request->subttlcost2[$k1]) ? $request->subttlcost2[$k1] : 0;
						$gwilsubtotal_inr = isset($request->gwilsubtotal_inr[$k1]) ? $request->gwilsubtotal_inr[$k1] : 0;
						$forwardingcostinr = isset($request->forwardingcostinr[$k1]) ? $request->forwardingcostinr[$k1] : 0;
						$coltotalprice = isset($request->coltotalprice[$k1]) ? $request->coltotalprice[$k1] : 0;
						$distr_margin = isset($request->distr_margin[$k1]) ? $request->distr_margin[$k1] : 0;
						...
						$subttlcost = ($subttlcost == '') ? 0 : floatval($subttlcost);
						$subttlcost2 = ($subttlcost2 == '') ? 0 : floatval($subttlcost2);
						$gwilsubtotal_inr = ($gwilsubtotal_inr == '') ? 0 : floatval($gwilsubtotal_inr);
						$forwardingcostinr = ($forwardingcostinr == '') ? 0 : floatval($forwardingcostinr);
						$coltotalprice = ($coltotalprice == '') ? 0 : floatval($coltotalprice);
						$distr_margin = ($distr_margin == '') ? 0 : floatval($distr_margin);
						...
						$prod_data = array(
							...
							'subttlcost' => $subttlcost,
							'subttlcost2' => $subttlcost2,
							'gwilsubtotal_inr' => $gwilsubtotal_inr,
							'forwardingcostinr' => $forwardingcostinr,
							'coltotalprice' => $coltotalprice,
							'distr_margin' => $distr_margin,
    ...
    public function show(string $id) //string $id
    {
		...
		if($quoteentry)
		{
			...
			$dealerid = isset($quoteentry->dealerid) ? $quoteentry->dealerid : 0;
		}
        ...
        return view('quotations.show', compact('quoteentry','materialDetails','dtobj','disc_res','cfg_res','maxshipvol','quote_type','sheet_type','shipment_type','cargo_type','cargoarr','currency_type','exp_cred_cost_thr_months','estd_miniman_cst','showtable','inr_usd','aed_usd','disc_arr','cust_locs','dealers'));
	...
	public function getfpproductsname(Request $request)
    {
        ...
        if($request->prodname_s)
        {
            ...
            $sql1->select(DB::Raw("TOP 50 PROD_DESC, PROD_CODE, PROD_RATE, PACK_SIZE"));
            //$sql1->where("PROD_DESC","LIKE","%".$request->prodname_s."%");
			$sql1->where(function ($query) use ($request) {
				$query->whereRaw("PROD_DESC LIKE '%".$request->prodname_s."%'")
					  ->orWhereRaw("PROD_CODE LIKE '%".$request->prodname_s."%'");
			});
            ...
            if($product_lists)
            {
                ...
                {
                    $prod_arr[] = array("value"=>$valobj->PROD_CODE.'$$$'.$valobj->PACK_SIZE, "label"=>$valobj->PROD_CODE.' - '.$valobj->PROD_DESC);
                }
	...
	public function getprddata(Request $request)
	{
		...
		$shipment_type = isset($request->shipment_type) ? $request->shipment_type : '';
		$currency_type = isset($request->currency_type) ? $request->currency_type : "USD";
		$forwardingcostinrset = isset($request->forwardingcostinr) ? (float) $request->forwardingcostinr : 0;
		$forwardingcostinrhid = isset($request->forwardingcostinrhid) ? $request->forwardingcostinrhid : 0;
		$cust_loc_val = isset($request->cust_loc_val) ? $request->cust_loc_val : 0;
		$cust_loc_val = (float) $cust_loc_val;
		$dealerid = isset($request->dealerid) ? $request->dealerid : 0;
		$ispalletized = 0;
		if($cargo_type == 'H')
		{
		...
		$israteupdate = isset($request->israteupdate) ? $request->israteupdate : 0;
		if($israteupdate)
		{
			$exchange_inr_usdnew = isset($request->exchange_inr_usdnew) ? $request->exchange_inr_usdnew : 0;
			$data_arr1 = array('para_value' => $exchange_inr_usdnew);
			
			if($exchange_inr_usdnew > 0)
			{
				DB::table('config_fields')->where('is_delete',0)->where('para_name','inr_usd')->update($data_arr1);
			}
		}
        ...
        if($request->prodcode_s)
        {
			...
			$isother = isset($request->isother) ? $request->isother : 0;
			...
			if($ship_vol_qty > 0)
			{
				//pack_fcl_nh
				$PriceWt = 0;
				$tmp_shipvol = $this->checkShipVol("Non-Haz","FCL", 0,$ttlshipvol,"Packed",$cons_act_weight);
			...
			if($ship_vol_qty > 0)
			{
				//pack_fcl_nh
				$PriceWt = 0;
				$tmp_shipvol = $this->checkShipVol("Non-Haz","LCL", 1,$ttlshipvol,"Packed",$cons_act_weight);
				
				if($tmp_shipvol < 10000)
				{
			...
			if($ship_vol_qty > 0)
			{
				...
				$tmp_shipvol = $this->checkShipVol("Non-Haz","FCL", 0,$ttlshipvol,"FOB",$cons_act_weight);
				//CargoType,ShipmentType,isPalletized,Weight,Sheetname
				...
				if($tmp_shipvol > 0)
				{
					...
					if($ispalletized == 0 && $cargo_type == 'NH' && $shipment_type == 'FCL')
					{
						if($forwardingcostinrhid == 1)
						{
							//$forwarding2 = $forwardingcostinrset;
						}
						//$forwardingcostinr = $forwarding2;
					}
			...
			if($ship_vol_qty > 0)
			{
				//pack_fcl_nh
				$PriceWt = 0;
				$tmp_shipvol = $this->checkShipVol("Non-Haz","LCL", 1,$ttlshipvol,"FOB",$cons_act_weight);
				if($tmp_shipvol < 10000)
				{
					...
					if($tmp_shipvol > 0)
					{
						...
						if($ispalletized == 0 && $cargo_type == 'NH' && $shipment_type == 'LCL')
						{
							if($forwardingcostinrhid == 1)
							{
								//$forwarding3 = $forwardingcostinrset;
							}
							//$forwardingcostinr = $forwarding3;
						}
			...
			if($ship_vol_qty > 0)
			{
				...
				$tmp_shipvol = $this->checkShipVol("Haz","FCL", 1,$ttlshipvol,"FOB",$cons_act_weight);
				//CargoType,ShipmentType,isPalletized,Weight,Sheetname
				...
				if($tmp_shipvol > 0)
				{
					...
					if($ispalletized == 1 && $cargo_type == 'H' && $shipment_type == 'FCL')
					{
						if($forwardingcostinrhid == 1)
						{
							//$forwarding4 = $forwardingcostinrset;
						}
						//$forwardingcostinr = $forwarding4;
					}
					...
					if($cost_pallet)
					{
						if($pallet_weight)
						{
							...
							$tmp_pl_weight = ceil($tmp_shipvol / $pallet_weight);
							$tmp_fob_fcl_h_inr = ($transport4 + ($cost_pallet * $tmp_pl_weight) + $forwarding4 + ($indhaz4 * $inr_usd)) / $tmp_shipvol;
						}
			...
			if($ship_vol_qty > 0)
			{
				//pack_fcl_nh
				$PriceWt = 0;
				$tmp_shipvol = $this->checkShipVol("Haz","LCL", 1,$ttlshipvol,"FOB",$cons_act_weight);
				if($tmp_shipvol < 10000)
				{
					...
					if($tmp_shipvol > 0)
					{
						...
						if($ispalletized == 1 && $cargo_type == 'H' && $shipment_type == 'LCL')
						{
							if($forwardingcostinrhid == 1)
							{
								//$forwarding5 = $forwardingcostinrset;
							}
							//$forwardingcostinr = $forwarding5;
						}
			...
			if($ship_vol_qty > 0)
			{
				...
				$tmp_shipvol = $this->checkShipVol("Non-Haz","FCL", 0,$ttlshipvol,"CFR",$cons_act_weight);
				//CargoType,ShipmentType,isPalletized,Weight,Sheetname
				...
				if($tmp_shipvol > 0)
				{
					...
					//echo 'debug'.' '.$sheet_type.' '.$cargo_type.' '.$shipment_type;die;
					if($ispalletized == 0 && $cargo_type == 'NH' && $shipment_type == 'FCL')
					{
						if($forwardingcostinrhid == 1)
						{
							//$forwarding6 = $forwardingcostinrset;
						}
						//$forwardingcostinr = $forwarding6;
					}
			...
			if($ship_vol_qty > 0)
			{
				//pack_fcl_nh
				$PriceWt = 0;
				$tmp_shipvol = $this->checkShipVol("Non-Haz","LCL", 1,$ttlshipvol,"CFR",$cons_act_weight);
				if($tmp_shipvol < 10000)
				{
					...
					if($tmp_shipvol > 0)
					{
						...
						if($ispalletized == 0 && $cargo_type == 'NH' && $shipment_type == 'LCL')
						{
							if($forwardingcostinrhid == 1)
							{
								//$forwarding7 = $forwardingcostinrset;
							}
							//$forwardingcostinr = $forwarding7;
						}
			...
			if($ship_vol_qty > 0)
			{
				...
				$tmp_shipvol = $this->checkShipVol("Haz","FCL", 1,$ttlshipvol,"CFR",$cons_act_weight);
				//CargoType,ShipmentType,isPalletized,Weight,Sheetname
				...
				if($tmp_shipvol > 0)
				{
					...
					if($ispalletized == 1 && $cargo_type == 'H' && $shipment_type == 'FCL')
					{
						if($forwardingcostinrhid == 1)
						{
							//$forwarding8 = $forwardingcostinrset;
						}
						//$forwardingcostinr = $forwarding8;
					}
					...
					if($cost_pallet)
					{
						if($pallet_weight)
						{
							...
							$tmp_pl_weight = ceil($tmp_shipvol / $pallet_weight);
							$tmp_cfr_fcl_h_inr = ($transport8 + ($cost_pallet * $tmp_pl_weight) + $forwarding8 + (($freight8 + $indhaz8) * $inr_usd)) / $tmp_shipvol;
						}
			...
			if($ship_vol_qty > 0)
			{
				//pack_fcl_nh
				$PriceWt = 0;
				$tmp_shipvol = $this->checkShipVol("Haz","LCL", 1,$ttlshipvol,"CFR",$cons_act_weight);
				if($tmp_shipvol < 10000)
				{
					...
					if($tmp_shipvol > 0)
					{
						...
						if($ispalletized == 1 && $cargo_type == 'H' && $shipment_type == 'LCL')
						{
							if($forwardingcostinrhid == 1)
							{
								//$forwarding9 = $forwardingcostinrset;
							}
							//$forwardingcostinr = $forwarding9;
						}
						...
						if($cost_pallet)
						{
							if($pallet_weight)
							{
								...
								$tmp_pl_weight = ceil($tmp_shipvol / $pallet_weight);
								$tmp_cfr_lcl_h_inr = ($transport9 + ($cost_pallet * $tmp_pl_weight) + $forwarding9 + ($tmp_pl_weight * $india_thc_cbm) + (($freight9 + $indhaz9) * $inr_usd)) / $tmp_shipvol;
							}
			...
			if($ship_vol_qty > 0)
			{
				...
				$tmp_shipvol = $this->checkShipVol("Non-Haz","FCL", 0,$ttlshipvol,"UAESide",$cons_act_weight);
				//CargoType,ShipmentType,isPalletized,Weight,Sheetname
			...
			if($ship_vol_qty > 0)
			{
				//pack_fcl_nh
				$PriceWt = 0;
				$tmp_shipvol = $this->checkShipVol("Non-Haz","LCL", 1,$ttlshipvol,"UAESide",$cons_act_weight);
				if($tmp_shipvol < 10000)
				{
			...
			if($ship_vol_qty > 0)
			{
				...
				$tmp_shipvol = $this->checkShipVol("Haz","FCL", 1,$ttlshipvol,"UAESide",$cons_act_weight);
				//CargoType,ShipmentType,isPalletized,Weight,Sheetname
				...
				if($tmp_shipvol > 0)
				{
					...
					$tmp_landed_fcl_h_inr = (($DelO12 + $Customs12 + $THC12 + $GatePass12 + $Insp12 + $UAEHaz12 + $Transport212) * $inr_usd) / $tmp_shipvol;
			...
			if($ship_vol_qty > 0)
			{
				//pack_fcl_nh
				$PriceWt = 0;
				$tmp_shipvol = $this->checkShipVol("Haz","LCL", 1,$ttlshipvol,"UAESide",$cons_act_weight);
				if($tmp_shipvol < 10000)
				{
					...
					if($tmp_shipvol > 0)
					{
						...
						$tmp_landed_lcl_h_inr = (($DelO13 + $Customs13 + (($tmp_pl_weight > $included_pallets) ? $uae_thc_pallet * $tmp_pl_weight : 0) + $THC13 + $GatePass13 + $Insp13 + $UAEHaz13 + $Transport213) * $inr_usd) / $tmp_shipvol;
			...
			if($ship_vol_qty > 0)
			{
				...
				$tmp_shipvol = $this->checkShipVol("Haz","FCL", 1,$ttlshipvol,"Packed",$cons_act_weight);
				//CargoType,ShipmentType,isPalletized,Weight,Sheetname
				...
				if($tmp_shipvol > 0)
				{
					...
					if($ispalletized == 1 && $cargo_type == 'H' && $shipment_type == 'FCL')
					{
						if($forwardingcostinrhid == 1)
						{
							//$forwarding15 = $forwardingcostinrset;
						}
						//$forwardingcostinr = $forwarding15;
					}
			...
			else
			{
				...
				if($sheet_type == 'FOB')
				{
					//FOB FCL H PALLETIZED
					//FOB FCL H PALLETIZED
					$t3 = ($tmp_fob_fcl_h_usd - $tmp_pk_fcl_h_usd);
				}
				else
				{
			...
			if($cust_loc_val > 0)
			{
				$cst_fwd_distr_buyer = $cust_loc_val;
			}
			...
			//$resp_arr['ttlcost_inc_finance'] = round($aa3,2);
			//echo 'debug '.$aq3.' => '.round($aq3,2,PHP_ROUND_HALF_DOWN).' => '.$ship_vol_qty;die;
			$subttlcost = round($aq3,2) * $ship_vol_qty;
			$resp_arr['subttlcost'] = round($subttlcost,2);
			//$resp_arr['subttlcost'] = round($t3,2);
			...
			$subttlcost2 = round($bb3,2) * $ship_vol_qty;
			$resp_arr['subttlcost2'] = round($subttlcost2,2);
			$resp_arr['tmp_shipvol'] = $tmp_shipvol;
			$gwilsubtotal_inr = $prod_rate * $ship_vol_qty;
			$resp_arr['gwilsubtotal_inr'] = round($gwilsubtotal_inr,2);
			//$resp_arr['forwardingcostinr'] = round($forwardingcostinr,2);
			$resp_arr['forwardingcostinr'] = 0;
			$resp_arr['disc_margin'] = $disc_margin;
		}
		
		return json_encode($resp_arr);
	}
	
	public function checkShipVol($haztype,$shiptype, $ispalletized,$ttlshipvol,$sheetname,$cons_act_weight)
	{
		if(!$cons_act_weight)
		{
			$tmp_shipvol = $this->getRates($haztype,$shiptype, $ispalletized,$ttlshipvol,$sheetname); 
		}
		else
		{
			$tmp_shipvol = $ttlshipvol;
		}
		return $tmp_shipvol;
	}
	...
	public function updateexchangerate(Request $request)
	{
		...
		/*$check_endpoint = 'https://openexchangerates.org/api/latest.json';
						
		$check_params = array('app_id'=>'0fdc30744bc54b56a2b784ad88015906');
		//$url_para = '&NAME1='.$vendorname.'&POSTCODE='.$pincode.'&KTOKK='.$vendorgroup;
		$url_check = $check_endpoint . '?' . http_build_query($check_params);
		...
		curl_setopt($ch_c, CURLOPT_URL, $url_check);

		//curl_setopt($ch_c, CURLOPT_FRESH_CONNECT, true);
		//curl_setopt($ch_c, CURLOPT_TIMEOUT, 0);
		curl_setopt($ch_c, CURLOPT_SSL_VERIFYPEER, FALSE);

		$result_ch = curl_exec($ch_c);
		$error_msg = array();
		if (curl_errno($ch_c)) {
			$error_msg = curl_error($ch_c);
		}

		if($result_ch === false)
		{
			$error_msg = 'Curl Error : '.curl_error($ch_c).' => CODE : '.curl_errno($ch_c);
		}
		else
		{
			
		   $result_ch2 = json_decode($result_ch,true); 
		   $sap_msg_ch = '';
		   
			
				//if(count($result_ch2) > 0)
				if($result_ch2)
				{
					$timestamp = isset($result_ch2['timestamp']) ? $result_ch2['timestamp'] : '';
					$basecurr =  isset($result_ch2['base']) ? $result_ch2['base'] : '';
					$rates_arr = isset($result_ch2['rates']) ? $result_ch2['rates'] : array();
					if($rates_arr)
					{
						$indcurr = isset($rates_arr['INR']) ? $rates_arr['INR'] : 0;
						$indcurr = ($indcurr == '') ? 0 : round($indcurr,4);
						$aedcurr = isset($rates_arr['AED']) ? $rates_arr['AED'] : 0;
						$aedcurr = ($aedcurr == '') ? 0 : round($aedcurr,4);
						
						$data_arr1 = array('para_value' => $indcurr);
						$data_arr2 = array('para_value' => $aedcurr);
						
						DB::table('config_fields')->where('is_delete',0)->where('para_name','inr_usd')->update($data_arr1);
						DB::table('config_fields')->where('is_delete',0)->where('para_name','aed_usd')->update($data_arr2);
						
						$apitimestamp = 'NULL';
						if($timestamp)
						{
							$apitimestamp = date('Y-m-d H:i:s', $timestamp);
						}
						
						$data_arr3 = array('isupdate' => 1, 'apitimestamp' => $apitimestamp);
						DB::table('config_exchange_update')->insert($data_arr3);
					}
				}
		}*/
		$indcurr = isset($request->indcurr) ? $request->indcurr : 0;
		$indcurr = ($indcurr == '' || $indcurr == 'undefined') ? 0 : round($indcurr,4);
		$aedcurr = isset($request->aedcurr) ? $request->aedcurr : 0;
		$aedcurr = ($aedcurr == '' || $aedcurr == 'undefined') ? 0 : round($aedcurr,4);
		$timestamp = isset($request->tmptimestamp) ? $request->tmptimestamp : '';
		...
		$data_arr3 = array('isupdate' => 1, 'apitimestamp' => $apitimestamp,'indcurr' => $indcurr, 'aedcurr' => $aedcurr);
		if($indcurr > 0)
		{
			DB::table('config_exchange_update')->insert($data_arr3);
		}
		
		$resp_arr = array('success' => 1);
		return json_encode($resp_arr);
	...
	public function updatecostfwddistbuyer(Request $request)
	{
		$cst_fwd_distr_buyer = isset($request->cst_fwd_distr_buyer) ? $request->cst_fwd_distr_buyer : 0;
		
			$cst_fwd_distr_buyer = ($cst_fwd_distr_buyer == '' || $cst_fwd_distr_buyer == 'undefined') ? 0 : round($cst_fwd_distr_buyer,4);
			
			
			$data_arr1 = array('para_value' => $cst_fwd_distr_buyer);
		
			
			if($cst_fwd_distr_buyer > 0)
			{
				DB::table('config_fields')->where('is_delete',0)->where('dealerid',$dealerid)->where('para_name','cst_fwd_distr_buyer')->update($data_arr1);
			}
	...
	public function setupparams(Request $request)
	{
		$resp_arr = array();
		$dealerid = isset($request->dealerid) ? $request->dealerid : 0;
		$cfg_res = DB::table('config_fields')->where('is_delete',0)->where('dealerid',$dealerid)->get();
		...
		if($cfg_res->count() > 0)
		{
			$para_arr = array('aed_usd','inr_usd','estd_miniman_cst','cst_fwd_distr_buyer','exp_cred_cost_thr_months');
			foreach($cfg_res as $res)
			{
				if(in_array($res->para_name, $para_arr)) {}
				else
				{
					$resp_arr[$res->para_name] = (($res->para_value) ? number_format($res->para_value, 2) : "");
				}
			}
