...
class GateEntryDetailController extends Controller
{
    ...
    public function index(Request $request)
    {
		...
		//REMOVE
		/*$id = 129;
		$gateentrynew = GateEntryDetail::where('id',$id)->first();		
		...
		if(isset($request->srch_gaterefno))
		{
			if($request->srch_gaterefno != '')
			{
				$srch_gaterefno = $request->srch_gaterefno;
				$sql1->whereRaw("gate_in_no like '%".$srch_gaterefno."%'");
			}
		}
		
		$srch_doctype = '';
		//echo 'TEST : '.$request->srch_gaterefno;die;
		if(isset($request->srch_doctype))
		{
			if($request->srch_doctype != '')
			{
				$srch_doctype = $request->srch_doctype;
				$sql1->where('doc_type_code',$srch_doctype);
			}
		}
		
		$srch_docno = '';
		//echo 'TEST : '.$request->srch_gaterefno;die;
		if(isset($request->srch_docno))
		{
			if($request->srch_docno != '')
			{
				$srch_docno = $request->srch_docno;
				$sql1->whereRaw("id = (select TOP 1 gate_entry_id from gate_entry_item_details where po_no = '".$srch_docno."' and is_delete = 0)");
			}
		}
		
		$srch_vendor = '';
		//echo 'TEST : '.$request->srch_gaterefno;die;
		if(isset($request->srch_vendor))
		{
			if($request->srch_vendor != '')
			{
				$srch_vendor = $request->srch_vendor;
				$sql1->whereRaw("vendor_name like '%".$srch_vendor."%'");
			}
		}
		
		$srch_status = '';
		//echo 'TEST : '.$request->srch_gaterefno;die;
		if(isset($request->srch_status))
		{
			if($request->srch_status != '')
			{
				$srch_status = $request->srch_status;
				$sql1->where('status',$srch_status);
			}
		}
		
		$from_gate_in_date = '';
		//echo 'TEST : '.$request->srch_gaterefno;die;
		if(isset($request->from_gate_in_date))
		{
			if($request->from_gate_in_date != '')
			{
				$from_gate_in_date = $request->from_gate_in_date;
			}
		}
		
		$from_gate_in_time = '';
		//echo 'TEST : '.$request->srch_gaterefno;die;
		if(isset($request->from_gate_in_time))
		{
			if($request->from_gate_in_time != '')
			{
				$from_gate_in_time = $request->from_gate_in_time;
			}
		}
		
		//if($from_gate_in_date || $from_gate_in_time)
		if($from_gate_in_date)
		{
			
			if($from_gate_in_date == '')
			{
				$from_gate_in_date = date('Y-m-d');
			}
			else
			{
				$from_gate_in_date = date('Y-m-d', strtotime($from_gate_in_date));
			}
			/*if($from_gate_in_time == '')
			{
				$from_gate_in_time = '00:00';
			}
			else
			{
				$from_gate_in_time = date('H:i', strtotime($from_gate_in_time));
			}*/
			
			$sql1->whereRaw("gate_in_date >= '".$from_gate_in_date."'");
			//$sql1->whereRaw("gate_in_time >= '".$from_gate_in_time."'");
		}
		
		$to_gate_in_date = '';
		//echo 'TEST : '.$request->srch_gaterefno;die;
		if(isset($request->to_gate_in_date))
		{
			if($request->to_gate_in_date != '')
			{
				$to_gate_in_date = $request->to_gate_in_date;
			}
		}
		...
		//if($to_gate_in_date || $to_gate_in_time)
		if($to_gate_in_date)
		{
			
			if($to_gate_in_date == '')
			{
				$to_gate_in_date = date('Y-m-d');
			}
			/*if($to_gate_in_time == '')
			{
			...
			$sql1->whereRaw("gate_in_date <= '".$to_gate_in_date."'");
			//$sql1->whereRaw("gate_in_time <= '".$to_gate_in_time."'");
		}
		
		$from_gate_out_date = '';
		//echo 'TEST : '.$request->srch_gaterefno;die;
		if(isset($request->from_gate_out_date))
		{
			if($request->from_gate_out_date != '')
			{
				$from_gate_out_date = $request->from_gate_out_date;
			}
		}
		
		$from_gate_out_time = '';
		//echo 'TEST : '.$request->srch_gaterefno;die;
		if(isset($request->from_gate_out_time))
		{
			if($request->from_gate_out_time != '')
			{
				$from_gate_out_time = $request->from_gate_out_time;
			}
		}
		
		//if($from_gate_out_time || $from_gate_out_time)
			
		if($from_gate_out_date)
		{
			
			if($from_gate_out_date == '')
			{
				$from_gate_out_date = date('Y-m-d');
			}
			else
			{
				$from_gate_out_date = date('Y-m-d', strtotime($from_gate_out_date));
			}
			/*if($from_gate_in_time == '')
			{
				$from_gate_in_time = '00:00';
			...
			$sql1->whereRaw("gate_out_date >= '".$from_gate_out_date."'");
			//$sql1->whereRaw("gate_in_time >= '".$from_gate_in_time."'");
		}
		
		$to_gate_out_date = '';
		//echo 'TEST : '.$request->srch_gaterefno;die;
		if(isset($request->to_gate_out_date))
		{
			if($request->to_gate_out_date != '')
			{
				$to_gate_out_date = $request->to_gate_out_date;
			}
		}
		
		$to_gate_out_time = '';
		//echo 'TEST : '.$request->srch_gaterefno;die;
		if(isset($request->to_gate_out_time))
		{
			if($request->to_gate_out_time != '')
			{
				$to_gate_out_time = $request->to_gate_out_time;
			}
		}
		
		//if($to_gate_out_date || $to_gate_out_time)
		if($to_gate_out_date)
		{
			
			if($to_gate_out_date == '')
			{
				$to_gate_out_date = date('Y-m-d');
			}
			/*if($to_gate_in_time == '')
			{
				$to_gate_in_time = '00:00';
			...
			$sql1->whereRaw("gate_in_date <= '".$to_gate_out_date."'");
			//$sql1->whereRaw("gate_in_time <= '".$to_gate_in_time."'");
		}
		...
		else if(in_array('StoreUser', $userRoles))
		{
			$store_stats = 1;
		...
		else
		{
			...
			if(Auth::user())
			{
				$roleIdsArray = Auth::user()->roles->pluck('id')->toArray();
				if($roleIdsArray)
				{
					$reddepts = DB::table('departments')->select('id')->whereIn('rolemapping',$roleIdsArray)->get();
					if($reddepts->count() > 0)
					{
						
						foreach($reddepts as $tmpdeptid)
						{
							$deptarr[] = $tmpdeptid->id;
						}
					}
				}
			}
			...
			if($deptarr)
			{
				...
				$sql1->where('status', '>=',$store_stats);
				$is_storeuser = true;
				$sql1->whereIn('dept_id',$deptarr);
			}
			
		}
		
		$gateEntries = $sql1->orderby('id','DESC')->paginate(20);
        ...
        return view('gateentries.index', compact('gateEntries','is_storeuser','show_plant_loc','srch_gaterefno','srch_doctype','srch_docno','srch_vendor','srch_status','from_gate_in_date','from_gate_in_time','to_gate_in_date','to_gate_in_time','from_gate_out_date','from_gate_out_time','to_gate_out_date','to_gate_out_time'));
    }
    ...
    public function create()
    {
        ...
        return view('gateentries.create', compact('dtobj','operator_name','plantsres','op_name','loc_code','loc_name','sel_loc','is_storeuser','deptlist','sel_dept','vehiclelist'));
    }
    ...
    public function store(Request $request)
    {
		...
		$this->save_containers($insert_id,$request);
			//
    ...
    public function show(GateEntryDetail $gateentry) //string $id
    {
        ...
        return view('gateentries.show', compact('gateentry','materialDetails','dtobj','seldoclbl','doc_no','location','dept_name','addr1','addr2','dept_incharge','containerDetails','selvehidlbl','seldellbl'));
    ...
    public function edit(GateEntryDetail $gateentry) //string $id
    {
        ...
        return view('gateentries.edit', compact('gateentry','materialDetails','dtobj','plantsres','op_name','loc_code','loc_name','sel_loc','is_storeuser','deptlist','sel_dept','vehiclelist','containerDetails'));
    }
	...
	public function generatepdf($gateentry,$isrep=0)
	{
		...
		$prefixn = 'gateprint';
		$pdf = PDF::loadView('gateentries.'.$prefixn,  compact('logo_img','gateentry','materialDetails','seldoclbl','doc_no','location','dept_name','addr1','addr2','dept_incharge','containerDetails','selvehidlbl','seldellbl','closed_by'));
