...
class QuotationController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('quotations.index', compact('quoteEntries','srch_quoterefno','from_quote_date','to_quote_date','disc_res','srch_prodname','srch_qty','srch_disctype'));
    }
    ...
    public function create()
    {
        ...
        return view('quotations.create', compact('dtobj','op_name','disc_res','cfg_res'));
    }
    ...
    public function show(string $id) //string $id
    {
        ...
        return view('quotations.show', compact('quoteentry','materialDetails','dtobj','disc_res','cfg_res'));
	...
	{
		$prod_rate = $inr_usd = $cost_pallet = $pallet_weight = $india_thc_cbm = $included_pallets = $uae_thc_pallet = $listprice_usd = $pack_fcl_nh = $fob_fcl_nh = $pack_lcl_nh_pl = $fob_fcl_h_pl = $fob_lcl_h_pl = $cif_fcl_nh = $cif_fcl_nh_tmp = $cif_lcl_nh_pl = $cif_lcl_nh_pl_tmp = $cif_fcl_h_pl = $cif_fcl_h_pl_tmp = $cif_lcl_h_pl = $cif_lcl_h_pl_tmp = $landed_fcl_nh = $landed_lcl_nh_pl = $landed_fcl_h_pl = $landed_lcl_h_pl = 0;
		$resp_arr = array(
			'prod_rate_inr' => 0, 
			'prod_rate_usd' => 0, 
			'unp_exwork_inr' => 0, 
        ...
        if($request->prodcode_s)
        {
			$sql1_res = DB::table($table_name_fp)
					->select(array('PROD_RATE'))
					->where('PROD_CODE',$request->prodcode_s)
					->where('is_delete',0)
					->first();
			if($sql1_res)
			{
				$prod_rate = $sql1_res->PROD_RATE;
			}
			...
			$resp_arr['prod_rate_inr'] = $prod_rate;
			$resp_arr['prod_rate_usd'] = number_format($listprice_usd,2);
			...
			$resp_arr['unp_exwork_usd'] = number_format($unp_exwork_usd_tmp, 2);
			...
			$resp_arr['pack_fcl_nh'] = number_format($pack_fcl_nh, 2);
			...
			if($ship_vol_qty > 0)
			{
				...
				if($ttlshipvol < 10000)
				{
					...
					if($tmp_shipvol > 0)
					{
						...
						if($cost_pallet)
						{
							if($pallet_weight)
							{
								...
								$tmp_pk_lcl_nh_inr = ($cost_pallet * $tmp_pl_weight) / $ship_vol_qty;
							}
					...
					$resp_arr['pack_lcl_nh_pl'] = number_format($pack_lcl_nh_pl, 2);
				}
				else
				{
			...
			$resp_arr['fob_fcl_nh'] = number_format($fob_fcl_nh, 2);
			...
			if($ship_vol_qty > 0)
			{
				...
				if($ttlshipvol < 10000)
				{
					...
					$resp_arr['fob_lcl_nh_pl'] = number_format($fob_lcl_nh_pl, 2);
				}
				else
				{
			...
			if($ship_vol_qty > 0)
			{
				...
				if($ttlshipvol < 10000)
				{
					...
					$resp_arr['fob_lcl_h_pl'] = number_format($fob_lcl_h_pl, 2);
				}
				else
				{
			...
			if($ship_vol_qty > 0)
			{
				...
				$resp_arr['cif_fcl_nh'] = number_format($cif_fcl_nh, 2);
			...
			if($ship_vol_qty > 0)
			{
				...
				if($ttlshipvol < 10000)
				{
					...
					$resp_arr['cif_lcl_nh_pl'] = number_format($cif_lcl_nh_pl, 2);
				}
				else
				{
			...
			if($ship_vol_qty > 0)
			{
				...
				$resp_arr['cif_fcl_h_pl'] = number_format($cif_fcl_h_pl, 2);
			...
			if($ship_vol_qty > 0)
			{
				...
				if($ttlshipvol < 10000)
				{
					...
					$resp_arr['cif_lcl_h_pl'] = number_format($cif_lcl_h_pl, 2);
				}
				else
				{
			...
			if($ship_vol_qty > 0)
			{
				...
				$resp_arr['landed_fcl_nh'] = number_format($landed_fcl_nh, 2);
			...
			if($ship_vol_qty > 0)
			{
				...
				if($ttlshipvol < 10000)
				{
					...
					$resp_arr['landed_lcl_nh_pl'] = number_format($landed_lcl_nh_pl, 2);
			...
			if($ship_vol_qty > 0)
			{
				...
				$resp_arr['landed_fcl_h_pl'] = number_format($landed_fcl_h_pl, 2);
			...
			if($ship_vol_qty > 0)
			{
				...
				if($ttlshipvol < 10000)
				{
					...
					$resp_arr['landed_lcl_h_pl'] = number_format($landed_lcl_h_pl, 2);
				}
				else
				{
			...
			$resp_arr['disc_val'] = number_format($disc_val,2);
