$(document).ready(function () {
	...
	if ($("#cons_act_weight").is(":checked")) 
	{
		$("#maxshipvol").text('-');
		$("#total_ship_vol_qty_h").val(0);
		if(document.getElementById("total_ship_vol_qty_db"))
		{
			$("#maxshipvol").text($("#total_ship_vol_qty_db").val());
		}
	}
	
	$('#cons_act_weight').on('change', function() {
		//console.log('debug');
		submitfrm2();
	 });
    ...
    $( "#"+elemname+rowno ).autocomplete({
      ...
      //source: base_url + '/getfpproductsname'
        ...
        select: function (event, ui) {
		  ...
		  $("#"+elemname+rowno).attr('title',ui.item.label);
...
function validatentryform()
{
	...
	{ 
		...
		let cust_loc = $("#cust_loc").val();
		$("#cust_loc2").val(cust_loc);
		let cust_loc_text = $("#cust_loc_text").val();
		$("#cust_loc_text2").val(cust_loc_text);
		let dealer_name = $("#dealer_name").val();
		$("#dealer_name2").val(dealer_name);
		let dealer_name_text = $("#dealer_name_text").val();
		$("#dealer_name_text2").val(dealer_name_text);
		if($("#cons_act_weight").is(":checked")) 
		{
			$("#cons_act_weight2").val(1);
		}
		else
		{
			$("#cons_act_weight2").val(0);
		}
...
{
	//console.log('updateexrate');
	exchange_inr_usdnew = $("#exchange_inr_usd").val();
	if(exchange_inr_usdnew == '' || exchange_inr_usdnew == 'null') { exchange_inr_usdnew = 0; }
	exchange_inr_usdnew = parseFloat(exchange_inr_usdnew);
	
	exchange_inr_aednew = $("#exchange_inr_aed").text();
	if(exchange_inr_aednew == '' || exchange_inr_aednew == 'null') { exchange_inr_aednew = 0; }
	exchange_inr_aednew = parseFloat(exchange_inr_aednew);
	
	const tmptimestampnew = Math.floor(Date.now() / 1000);
	
	updateexchangeratefunc(exchange_inr_usdnew,exchange_inr_aednew,tmptimestampnew,1);
	
	
}

function updatefwdcost(rowno)
{
	$("#forwardingcostinrhid"+rowno).val(1);
	setshiptoqty(rowno);
}

function setshiptoqty(rowno,israteupdate=0)
{
	...
	else
	{
		...
		var calltypdisc = 0;
		fetchproddet_calc(rowno,prodcode,calltypdisc,0,israteupdate);
		fetchproddet_calc_others(rowno);
		$(document).ajaxStop(function() {
			// This function runs when ALL AJAX requests have finished
			//console.log("All AJAX requests are complete.");
...
function fetchproddet_calc(rowno,prodcode,calltypdisc=0,isother=0,israteupdate=0)
{
	...
	if(prodcode != '')
	{
		...
		var cst_fwd_distr_buytmp = $("#cst_fwd_distr_buyer").val();
		if(cst_fwd_distr_buytmp == '' || cst_fwd_distr_buytmp == 'null') { cst_fwd_distr_buytmp = 0; }
		cst_fwd_distr_buytmp = parseFloat(cst_fwd_distr_buytmp);
		
		cst_fwd_distr_buytmp = convertcstfwdrate(cst_fwd_distr_buytmp);
		cst_fwd_distr_buytmp = cst_fwd_distr_buytmp.toFixed(2);
		
		var cust_loc_val = $("#cust_loc").val();
		if(cust_loc_val == '') { cust_loc_val = 0; }
		cust_loc_val = parseFloat(cust_loc_val);
		...
		if(cust_loc_val > 0)
		{
			cst_fwd_distr_buytmp = convertcstfwdrate(cust_loc_val);
			cst_fwd_distr_buytmp = cst_fwd_distr_buytmp.toFixed(2);
		}
		
		//$("#forwardingcostinr"+rowno).val(cst_fwd_distr_buytmp);
		//$("#forwardingcostinrhid"+rowno).val(0);
		
		//var forwardingcostinr = $("#forwardingcostinr"+rowno).val();
		//if(forwardingcostinr == '' || forwardingcostinr == 'null') { forwardingcostinr = 0; }
		//forwardingcostinr = parseFloat(forwardingcostinr);
		
		//var forwardingcostinrhid = $("#forwardingcostinrhid"+rowno).val();
		...
		$.ajax({
				  ...
				  data: {
					...
					cons_act_weight:cons_act_weight,
					isother:isother,
					israteupdate:israteupdate,
					exchange_inr_usdnew:exchange_inr_usdnew,
					shipment_type:shipment_type,
					forwardingcostinr:cst_fwd_distr_buytmp,
					forwardingcostinrhid:cst_fwd_distr_buytmp,
					cust_loc_val:cust_loc_val,
					dealerid:dealerid
					
					//forwardingcostinr:forwardingcostinr,
					//forwardingcostinrhid:forwardingcostinrhid
				  },
				  success: function( data ) {
					//console.log( data );
					...
					var tmpprd = $("#prdcd"+rowno).val();
					
					var distr_margin = data.disc_margin;
					if(distr_margin == '' || distr_margin == 'null' || typeof distr_margin === 'undefined') 
					{ 
						distr_margin = 0; 
					}
					distr_margin = parseFloat(distr_margin);

					if(tmpprd != '')
					{
						$("#distr_margin"+rowno).val(distr_margin.toFixed(2));
					}
					else
					{
						$("#distr_margin"+rowno).val('');
					}
					...
					list_rate_usd = convertusdrate(list_rate_usd);
					...
					list_rate_usd_total = convertusdrate(list_rate_usd_total);
					...
					if(cif_lcl_h_pl != "Too Large")
					{
						...
						cif_lcl_h_pl = convertusdrate(cif_lcl_h_pl);
					}
					...
					landed_fcl_nh = convertusdrate(landed_fcl_nh);
					...
					if(landed_lcl_nh_pl != "Too Large")
					{
						...
						landed_lcl_nh_pl = convertusdrate(landed_lcl_nh_pl);
					}
					...
					landed_fcl_h_pl = convertusdrate(landed_fcl_h_pl);
					...
					if(landed_lcl_h_pl != "Too Large")
					{
						...
						landed_lcl_h_pl = convertusdrate(landed_lcl_h_pl);
					}
					...
					ttlcost_inc_finance = convertusdrate(ttlcost_inc_finance);
					...
					recom_dis_sp_to_buyer = convertusdrate(recom_dis_sp_to_buyer);
					...
					recom_sp_aft_credit_miniman = convertusdrate(recom_sp_aft_credit_miniman);
					
					var subttlcost = data.subttlcost;
					if(subttlcost == '' || subttlcost == 'null') { subttlcost = 0; }
					subttlcost = parseFloat(subttlcost);
					subttlcost = convertusdrate(subttlcost);
					
					var subttlcost2 = data.subttlcost2;
					if(subttlcost2 == '' || subttlcost2 == 'null') { subttlcost2 = 0; }
					subttlcost2 = parseFloat(subttlcost2);
					subttlcost2 = convertusdrate(subttlcost2);
					
					var gwilsubtotal_inr = data.gwilsubtotal_inr;
					if(gwilsubtotal_inr == '' || gwilsubtotal_inr == 'null') { gwilsubtotal_inr = 0; }
					gwilsubtotal_inr = parseFloat(gwilsubtotal_inr);
					
					//var forwardingcostinr = data.forwardingcostinr;
					//if(forwardingcostinr == '' || forwardingcostinr == 'null') { forwardingcostinr = 0; }
					//forwardingcostinr = parseFloat(forwardingcostinr);
					
					var tmp_shipvol = data.tmp_shipvol;
					if(tmp_shipvol == '' || tmp_shipvol == 'null') { tmp_shipvol = '-'; }
					
					if ($("#cons_act_weight").is(":checked")) 
					{
						//$("#maxshipvol").text('-');
						$("#maxshipvol").text(tmp_shipvol);
						$("#total_ship_vol_qty_h").val(tmp_shipvol);
					}
					else
					{
						if(cargo_type == 'NH')
						{
							$("#maxshipvol").text('15000');
							$("#total_ship_vol_qty_h").val(15000);
						}
						else if(cargo_type == 'H')
						{
							$("#maxshipvol").text('10000');
							$("#total_ship_vol_qty_h").val(10000);
						}
						else
						{
							$("#maxshipvol").text('-');
					...
					$("#listprice_inr_unit_span"+rowno).text(displayformattedvalue(list_rate_inr)); //newVal.toFixed(2)
					//var list_rate_inr_f = list_rate_inr.toFixed(2);
					//$("#listprice_inr_unit_span"+rowno).text((list_rate_inr_f.toLocaleString('en-IN')));
					
					$("#listprice_usd_unit"+rowno).val(list_rate_usd);
					$("#listprice_usd_unit_span"+rowno).text(displayformattedvalue(list_rate_usd));
					//var list_rate_usd_f = list_rate_usd.toFixed(2);
					//$("#listprice_usd_unit_span"+rowno).text((list_rate_usd_f.toLocaleString('en-IN')));
					
					$("#listprice_inr"+rowno).val(list_rate_inr_total);
					$("#listprice_inr_span"+rowno).text(displayformattedvalue(list_rate_inr_total));
					//var list_rate_inr_total_f = list_rate_inr_total.toFixed(2);
					//$("#listprice_inr_span"+rowno).text((list_rate_inr_total_f.toLocaleString('en-IN')));
					
					$("#listprice_usd"+rowno).val(list_rate_usd_total);
					$("#listprice_usd_span"+rowno).text(displayformattedvalue(list_rate_usd_total));
					//var list_rate_usd_total_f = list_rate_usd_total.toFixed(2);
					//$("#listprice_usd_span"+rowno).text((list_rate_usd_total_f.toLocaleString('en-IN')));
					
					$("#unp_exwork_inr"+rowno).val(unp_exwork_inr);
					$("#unp_exwork_inr_span"+rowno).text(displayformattedvalue(unp_exwork_inr));
					//var unp_exwork_inr_f = unp_exwork_inr.toFixed(2);
					//$("#unp_exwork_inr_span"+rowno).text((unp_exwork_inr_f.toLocaleString('en-IN')));
					
					$("#unp_exwork_usd"+rowno).val(unp_exwork_usd);
					$("#unp_exwork_usd_span"+rowno).text(displayformattedvalue(unp_exwork_usd));
					//var unp_exwork_usd_f = unp_exwork_usd.toFixed(2);
					//$("#unp_exwork_usd_span"+rowno).text((unp_exwork_usd_f.toLocaleString('en-IN')));
					
					$("#pack_fcl_nh"+rowno).val(pack_fcl_nh);
					$("#pack_fcl_nh_span"+rowno).text(displayformattedvalue(pack_fcl_nh));
					//var pack_fcl_nh_f = pack_fcl_nh.toFixed(2);
					//$("#pack_fcl_nh_span"+rowno).text((pack_fcl_nh_f.toLocaleString('en-IN')));
					...
					{
						$("#pack_lcl_pallet_span"+rowno).text(displayformattedvalue(pack_lcl_nh_pl));
						//var pack_lcl_nh_pl_f = pack_lcl_nh_pl.toFixed(2);
						//$("#pack_lcl_pallet_span"+rowno).text((pack_lcl_nh_pl_f.toLocaleString('en-IN')));
					}
					
					$("#fob_fcl_nh"+rowno).val(fob_fcl_nh);
					$("#fob_fcl_nh_span"+rowno).text(displayformattedvalue(fob_fcl_nh));
					//var fob_fcl_nh_f = fob_fcl_nh.toFixed(2);
					//$("#fob_fcl_nh_span"+rowno).text((fob_fcl_nh_f.toLocaleString('en-IN')));
					...
					{
						$("#fob_lcl_nh_pl_span"+rowno).text(displayformattedvalue(fob_lcl_nh_pl));
						//var fob_lcl_nh_pl_f = fob_lcl_nh_pl.toFixed(2);
						//$("#fob_lcl_nh_pl_span"+rowno).text((fob_lcl_nh_pl_f.toLocaleString('en-IN')));
					}
					
					
					$("#fob_fcl_h_pl"+rowno).val(fob_fcl_h_pl);
					$("#fob_fcl_h_pl_span"+rowno).text(displayformattedvalue(fob_fcl_h_pl));
					//var fob_fcl_h_pl_f = fob_fcl_h_pl.toFixed(2);
					//$("#fob_fcl_h_pl_span"+rowno).text((fob_fcl_h_pl_f.toLocaleString('en-IN')));
					...
					{
						$("#fob_lcl_h_pl_span"+rowno).text(displayformattedvalue(fob_lcl_h_pl));
						//var fob_lcl_h_pl_f = fob_lcl_h_pl.toFixed(2);
						//$("#fob_lcl_h_pl_span"+rowno).text((fob_lcl_h_pl_f.toLocaleString('en-IN')));
					}
					
					$("#cif_fcl_nh"+rowno).val(cif_fcl_nh);
					$("#cif_fcl_nh_span"+rowno).text(displayformattedvalue(cif_fcl_nh));
					//var cif_fcl_nh_f = cif_fcl_nh.toFixed(2);
					//$("#cif_fcl_nh_span"+rowno).text((cif_fcl_nh_f.toLocaleString('en-IN')));
					...
					{
						$("#cif_lcl_nh_pl_span"+rowno).text(displayformattedvalue(cif_lcl_nh_pl));
						//var cif_lcl_nh_pl_f = cif_lcl_nh_pl.toFixed(2);
						//$("#cif_lcl_nh_pl_span"+rowno).text((cif_lcl_nh_pl_f.toLocaleString('en-IN')));
					}
					
					$("#cif_fcl_h_pl"+rowno).val(cif_fcl_h_pl);
					$("#cif_fcl_h_pl_span"+rowno).text(displayformattedvalue(cif_fcl_h_pl));
					//var cif_fcl_h_pl_f = cif_fcl_h_pl.toFixed(2);
					//$("#cif_fcl_h_pl_span"+rowno).text((cif_fcl_h_pl_f.toLocaleString('en-IN')));
					...
					{
						$("#cif_lcl_h_pl_span"+rowno).text(displayformattedvalue(cif_lcl_h_pl));
						//var cif_lcl_h_pl_f = cif_lcl_h_pl.toFixed(2);
						//$("#cif_lcl_h_pl_span"+rowno).text((cif_lcl_h_pl_f.toLocaleString('en-IN')));
					}
					
					$("#landed_fcl_nh"+rowno).val(landed_fcl_nh);
					$("#landed_fcl_nh_span"+rowno).text(displayformattedvalue(landed_fcl_nh));
					//var landed_fcl_nh_f = landed_fcl_nh.toFixed(2);
					//$("#landed_fcl_nh_span"+rowno).text((landed_fcl_nh_f.toLocaleString('en-IN')));
					...
					{
						$("#landed_lcl_nh_pl_span"+rowno).text(displayformattedvalue(landed_lcl_nh_pl));
						//var landed_lcl_nh_pl_f = landed_lcl_nh_pl.toFixed(2);
						//$("#landed_lcl_nh_pl_span"+rowno).text((landed_lcl_nh_pl_f.toLocaleString('en-IN')));
					}
					
					$("#landed_fcl_h_pl"+rowno).val(landed_fcl_h_pl);
					$("#landed_fcl_h_pl_span"+rowno).text(displayformattedvalue(landed_fcl_h_pl));
					//var landed_fcl_h_pl_f = landed_fcl_h_pl.toFixed(2);
					//$("#landed_fcl_h_pl_span"+rowno).text((landed_fcl_h_pl_f.toLocaleString('en-IN')));
					...
					{
						$("#landed_lcl_h_pl_span"+rowno).text(displayformattedvalue(landed_lcl_h_pl));
						//var landed_lcl_h_pl_f = landed_lcl_h_pl.toFixed(2);
						//$("#landed_lcl_h_pl_span"+rowno).text((landed_lcl_h_pl_f.toLocaleString('en-IN')));
					}
					
					//$("#forwardingcostinr"+rowno).val(forwardingcostinr);
					
					$("#ttlcost_inc_finance"+rowno).val(ttlcost_inc_finance);
					$("#ttlcost_inc_finance_span"+rowno).text(displayformattedvalue(ttlcost_inc_finance));
					//var ttlcost_inc_finance_f = ttlcost_inc_finance.toFixed(2);
					//$("#ttlcost_inc_finance_span"+rowno).text((ttlcost_inc_finance_f.toLocaleString('en-IN')));
					
					$("#recom_dis_sp_to_buyer"+rowno).val(recom_dis_sp_to_buyer);
					$("#recom_dis_sp_to_buyer_span"+rowno).text(displayformattedvalue(recom_dis_sp_to_buyer));
					//var recom_dis_sp_to_buyer_f = recom_dis_sp_to_buyer.toFixed(2);
					//$("#recom_dis_sp_to_buyer_span"+rowno).text((recom_dis_sp_to_buyer_f.toLocaleString('en-IN')));
					
					$("#recom_sp_aft_credit_miniman"+rowno).val(recom_sp_aft_credit_miniman);
					$("#recom_sp_aft_credit_miniman_span"+rowno).text(displayformattedvalue(recom_sp_aft_credit_miniman));
					//var recom_sp_aft_credit_miniman_f = recom_sp_aft_credit_miniman.toFixed(2);
					//$("#recom_sp_aft_credit_miniman_span"+rowno).text((recom_sp_aft_credit_miniman_f.toLocaleString('en-IN')));
					
					$("#subttlcost"+rowno).val(subttlcost);
					$("#subttlcost_span"+rowno).text(displayformattedvalue(subttlcost));
					
					//var subttlcost_f = subttlcost.toFixed(2);
					//$("#subttlcost_span"+rowno).text((subttlcost_f.toLocaleString('en-IN')));
					
					$("#subttlcost2"+rowno).val(subttlcost2);
					$("#subttlcost2_span"+rowno).text(displayformattedvalue(subttlcost2));
					//var subttlcost2_f = subttlcost2.toFixed(2);
					//$("#subttlcost2_span"+rowno).text((subttlcost2_f.toLocaleString('en-IN')));
					
					$("#gwilsubtotal_inr"+rowno).val(gwilsubtotal_inr);
					//$("#gwilsubtotal_inr_span"+rowno).text(gwilsubtotal_inr.toFixed(2));
					$("#gwilsubtotal_inr_span"+rowno).text(displayformattedvalue(gwilsubtotal_inr));
...
{	
	if(selval == '')
	{
		...
		{
			listprice_inr = parseFloat(listprice_inr);
		}
		...
		{
			listprice_usd = parseFloat(listprice_usd);
		}
...
function convertusdrate(curValue)
{
	var currency_type = $("#currency_type").val();
	...
	$('input[name="prdcd[]"]').each(function(){
		...
		else
		{
			...
			if(rownoby != rowno)
			{
				...
				fetchproddet_calc(rowno,prodcode,calltypdisc,1);
				$(document).ajaxStop(function() {
					// This function runs when ALL AJAX requests have finished
					//console.log("All AJAX requests are complete.");
					setTotalQuote();
...
{ console.log('exporting ....');
	...
	$clonedTable.find('tr').each(function(rowIndex, rowElement) {
		...
		trElement.find("td.savebtns").each(function(cellIndex, cellElement) {
			$(cellElement).remove();
		});
		...
		trElement.find(tmpsel1).each(function(cellIndex, cellElement) {
		  ...
		  {
			  if(elemid == 'recomdistsptobuyer' || elemid == 'oncent recomdistsptobuyer' || elemid == 'coltotalprice' || elemid == 'oncent coltotalprice' || elemid == 'ttlcostfont coltotalprice')
			  {
			  ...
			  else if(elemid == 'colforcust' || elemid == 'oncent colforcust' || elemid == 'ttlcostfont colforcust')
			  {
				  if(quote_type == 'Dealer')
				  {
					  $(cellElement).remove();
	...
	var clonedTableColCount = getColumnCount($clonedTable);
	//console.log('Clomns : '+clonedTableColCount);

	//Add Filter columns on top
	let qtype = $("#quote_type").val();
	let sheet_type = $("#sheet_type").val();
	let shipment_type = $("#shipment_type").val();
	let cargo_type = $("#cargo_type").val();
	let currency_type = $("#currency_type").val();
	var newRowHtml = '<tr><td style="font-weight:bold;">Quote For:</td><td>'+qtype+'</td><td style="font-weight:bold;">Sheet Type:</td><td>'+sheet_type+'</td><td style="font-weight:bold;">Shipment Type:</td><td>'+shipment_type+'</td><td style="font-weight:bold;">Cargo Type:</td><td>'+cargo_type+'</td><td style="font-weight:bold;">Currency:</td><td>'+currency_type+'</td></tr>';
	var lbl1 = '';
	var lbl1val = '';
	if(qtype == 'Customer')
	{
		lbl1 = 'Customer Location:';
		lbl1val = $("#cust_loc option:selected").text();
	}
	else
	{
		lbl1 = 'Dealer';
		lbl1val = $("#dealer_name option:selected").text();
	}
	let exp_cred_cost_thr_months = $("#exp_cred_cost_thr_months").val();
	let estd_miniman_cst = $("#estd_miniman_cst").val();
	let exchange_inr_usd = $("#exchange_inr_usd").val();
	let exchange_inr_aed = $("#exchange_inr_aed").text();

	newRowHtml += '<tr><td style="font-weight:bold;">'+lbl1+'</td><td>'+lbl1val+'</td><td style="font-weight:bold;">Exp. Cred. Cost(%):</td><td>'+exp_cred_cost_thr_months+'</td><td style="font-weight:bold;">Estd MiniMan Cst(%):</td><td>'+estd_miniman_cst+'</td><td style="font-weight:bold;">INR/USD:</td><td>'+exchange_inr_usd+'</td><td style="font-weight:bold;">AED/USD:</td><td>'+exchange_inr_aed+'</td></tr>';

	newRowHtml += '<tr><td colspan="'+clonedTableColCount+'">&nbsp;</td></tr>';
    
    $clonedTable.find("thead").prepend(newRowHtml);
	//Add Filter columns on top
...
{
    var colCount = 0;
    // Target the first row and all its direct children (th or td)
    $table.find('tr:first').children('th, td').each(function() {
        // Use the native DOM property .colSpan, which returns 1 if not explicitly set
        colCount += this.colSpan || 1; 
    });
    return colCount;
}
...
function setinputvalues(clonedTable)
{
	...
	// clonedTable.find('input[name="forwardingcostinr[]"]').each(function() {
	// 	$(this).after($(this).val());
    // });
	clonedTable.find('input[name="distr_margin[]"]').each(function() {
		$(this).after($(this).val());
    });
	...
	clonedTable.find('select[name="disc_type[]"]').each(function() {
		...
		var selectedText = $("#"+discid+" option:selected").text();
		$(this).after(selectedText);
    });
...
function changecurrencyvalues(currencyval)
{
	...
	$('.currencyspan').each(function(index) {
		...
		if(curValue != '')
		{
			if(curValue != 'Too Large')
			{
				curValue = curValue.replace(/,/g, '').replace('.', '.');
				curValue = (curValue=='' || curValue=='-') ? 0 : parseFloat(curValue);
				...
				$(this).text(displayformattedvalue(newVal));
				//var newVal_f = newVal.toFixed(2);
				//$(this).text((newVal_f.toLocaleString('en-IN')));
			}
		}
	});
	...
	$('.currencyinp').each(function(index) {
		...
		if(curValue != 'Too Large')
		{
			...
			{
				newVal = curValue * exrate;
				if(elemname == 'forwardingcostinr[]')
				{
					
					// let cst_fwd_distr_buytmp2 = $("#cst_fwd_distr_buyer").val();
					// if(cst_fwd_distr_buytmp2 == '') { cst_fwd_distr_buytmp2 = 0; }
					// cst_fwd_distr_buytmp2 = parseFloat(cst_fwd_distr_buytmp2);
					// newVal = cst_fwd_distr_buytmp2;
				}
			}
			else
			{
...
{
	var dealer_name_text = $("#dealer_name option:selected").text();
	$("#dealer_name_text").val(dealer_name_text);

	if(sheet_type == '') 
	{
		if(quotetyp == 'Customer')
		{
			sheet_type = 'Delivered';
		}
		else
		{
			sheet_type = 'Ex Works';
...
function commonchange()
{
	...
	let shipment_type = $("#shipment_type").val();
	let cargo_type = $("#cargo_type").val();
	setuptablecols(quote_type,sheet_type,shipment_type,cargo_type);
}

function setcustloc(elemid)
{
	var cust_loc_text = $("#"+elemid +" option:selected").text();
	$("#cust_loc_text").val(cust_loc_text);
	
	submitfrm2();
}

function setdealername(elemid)
{
	var dealer_name_text = $("#"+elemid +" option:selected").text();
	$("#dealer_name_text").val(dealer_name_text);
	setupparams();
	submitfrm2();
}
...
function setupparams()
{
	...
	console.log('setting up params for '+dealerid);

	var frm_action3 = "/setupparams";
	$.ajax({
		  url: frm_action3,
		  type: 'GET',
		  dataType: "json",
		  ...
		  success: function( data ) {
		  	var para_length = Object.keys(data).length;
		  	if(para_length > 0)
		  	{
		  		$.each(data, function(parakey, paravalue) {
		  			if(document.getElementById("p_"+parakey))
		  			{
		  				$("#p_"+parakey).text(paravalue);
		  			}
				});
		  	}
...
{
	var gwilsubtotalcost = 0;
	$('.gwilsubtotalcls').each(function(index) {
		let tmpsubtt3 = $(this).val();
		if(tmpsubtt3 == '') { tmpsubtt3 = 0; }
		tmpsubtt3 = parseFloat(tmpsubtt3);
		
		gwilsubtotalcost += tmpsubtt3;
	});
	
	$("#gwiltotal_inr").val(gwilsubtotalcost);
	$("#gwiltotal_inr_span").text(displayformattedvalue(gwilsubtotalcost));
	//var gwilsubtotalcost_f = gwilsubtotalcost.toFixed(2);
	//$("#gwiltotal_inr_span").text(gwilsubtotalcost_f.toLocaleString('en-IN'));
	...
	$("#quotettlcost_span").text(displayformattedvalue(qtttlcost));
	//var qtttlcost_f = qtttlcost.toFixed(2);
	//$("#quotettlcost_span").text(qtttlcost_f.toLocaleString('en-IN'));
	...
	$("#quotettlcost2_span").text(displayformattedvalue(qtttlcost2));
	//var qtttlcost2_f = qtttlcost2.toFixed(2);
	//$("#quotettlcost2_span").text(qtttlcost2_f.toLocaleString('en-IN'));
	
	var colexworksttlp = 0;
	$('.colexworksttlp').each(function(index) {
		let tmpcolexworksttlp = $(this).val();
		if(tmpcolexworksttlp == '') { tmpcolexworksttlp = 0; }
		tmpcolexworksttlp = parseFloat(tmpcolexworksttlp);
		
		colexworksttlp += tmpcolexworksttlp;
	});
	
	$("#colgtotalprice").val(colexworksttlp);
	$("#colgtotalprice_span").text(displayformattedvalue(colexworksttlp));
}
			...
			$.get('https://openexchangerates.org/api/latest.json', {app_id: '0fdc30744bc54b56a2b784ad88015906'}, function(data) {
				...
				if(data.rates !== 'undefined')
				{
					...
					if(curr_rates.INR !== 'undefined')
					{
						...
						$("#exchange_inr_usd").val(curr_rates.INR);
						if(curr_rates.AED !== 'undefined')
						{
							$("#exchange_inr_aed").text(curr_rates.AED);
						}
						updateexchangeratefunc(curr_rates.INR,curr_rates.AED,tmptimestamp);
...
function updateexchangeratefunc(curr_rates_INR,curr_rates_AED,tmptimestamp,ismanual=0)
{
	var frm_action2 = "/updateexchangerate";
	$.ajax({
		  url: frm_action2,
		  type: 'GET',
		  dataType: "json",
		  data: {
			indcurr: curr_rates_INR,
			aedcurr: curr_rates_AED,
			tmptimestamp: tmptimestamp
		  },
		  success: function( data ) {
			console.log( data );
			if(ismanual == 1)
...
function updatecostfwddistbuyer()
{
	cst_fwd_distr_buyer = $("#cst_fwd_distr_buyer").val();
	if(cst_fwd_distr_buyer == '' || cst_fwd_distr_buyer == 'null') { cst_fwd_distr_buyer = 0; }
	cst_fwd_distr_buyer = parseFloat(cst_fwd_distr_buyer);
	
	var frm_action2 = "/updatecostfwddistbuyer";
	$.ajax({
		  url: frm_action2,
		  type: 'GET',
		  dataType: "json",
		  data: {
			cst_fwd_distr_buyer: cst_fwd_distr_buyer,
		  },
		  success: function( data ) {
			console.log( data );
...
function displayformattedvalue(itemvalue)
{
	const formattedValue = itemvalue.toLocaleString('en-IN', {
...
function convertcstfwdrate(curValue)
{
	var currency_type = $("#currency_type").val();
	...
	else
	{
		var newVal = 0;
		var ex_usd_aed = $("#exchange_inr_aed").text();
		var exrate = (ex_usd_aed=='' || typeof ex_usd_aed === 'undefined') ? 0 : parseFloat(ex_usd_aed);
