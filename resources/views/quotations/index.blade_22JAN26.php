    ...
    <table class="table table-bordered" style="width:70%;margin-top:10px;">
        ...
        @foreach($quoteEntries as $k1 => $entry)
            ...
            <tr class="<?php echo $clsname;?>">
				...
				<td>{{ $entry->quote_type }}</td>
				<td>{{ $entry->sheet_type }}</td>
				<td>{{ $entry->shipment_type }}</td>
				<td><?php echo isset($cargoarr[$entry->cargo_type]) ? $cargoarr[$entry->cargo_type] : '';?></td>
				<td style="float:right;border:none;">{{ $entry->total_shipment_vol }}</td>
				<td>{{ $entry->name }}</td>
                <td>
				<?php
				$gtdt = '';
